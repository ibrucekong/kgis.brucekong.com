...
require([
  ...
  // 控制左侧菜单栏距离顶部的高度
  if (IsPC()) {
    $("#content").css("margin-right", 270);
    $(".sidenav.nav.nav-list.list.well.page").css("top", 52);
    $(".sidenav.nav.nav-list.list.well.page").css("margin-bottom", 0);
    $(".form-control.search").addClass("ksearch");
  }else {
    $("#content").css("margin-right", 0);
    $(".sidenav.nav.nav-list.list.well.page").css("top", 85)
    $(".sidenav.nav.nav-list.list.well.page").css("margin-bottom", 0);
    $(".form-control.search").removeClass("ksearch")
  }

  // 监听document滚动条的滚动事件监听
...
// 左侧菜单宽度变化，自动适配地图宽度，也解决了在F12中手机界面和PC界面切换时的bug
  ...
  var isShow = false;
  // 点击左上角的菜单开关按钮，显示隐藏左侧的菜单——仅在手机中适用
  $("#menuBtn").click(function () {

    if($("#sidenav").css("left") === "0px"){
      // 隐藏菜单
      $("#menuBtn > span").removeClass("glyphicons-remove");
      $("#menuBtn > span").addClass("glyphicon-menu-hamburger");

      $("#sidenav").animate({left:"-228px"});
      $(".sidenav-search").animate({left:"-228px"});
      $("#scrollingNav ul").animate({left:"-228px"});
      ...
      if (isShow){
        ...
        $(".sidenav-search").animate({left:"0"});
        $("#scrollingNav ul").animate({left:"0"});
  ...
  function resizeContent() {
    if (IsPC()) {
      // PC
      $("#content").css("margin-left", 270);
      $("#sidenav").show();
    } else {
      // phone
      $("#content").css("margin-left", 0);
      $("#sidenav").hide();
      if($("#sidenav").css("left")== "-228px"){
        $("#sidenav").show();
        $("#sidenav").animate({left:"0px"});
        $(".sidenav-search").animate({left:"0"});
        $("#scrollingNav ul").animate({left:"0"});
  ...
  $("#klogo").load(function () {
    ...
    if (IsPC()){
      ...
      $("#cus-title").css("display", "none");
      $("#klogo").css("height" ,84);
      $("#klogo").css("width" ,getImgWidth(84));
    } else {
      $("#cus-title").css("display", "block");
      $(".back_to_home").css("display", "none");
      $(".back_to_top").css("display", "none");

      // 判断img的width
      if ($("#klogo").width() >= $("#project").width()) {
        var w = $("#project").width() - 20;
        $("#klogo").css("width", w);
        $("#klogo").css("height" ,getImgHeight(w));
      }
    }
  })
  ...
  // 获取logo等比例的高度
  function getImgHeight(w) {
    var oWidth = 448;
    var oHeight = 133;
    var h = w * oHeight / oWidth;
    return h.toFixed(0);
  }

  // 获取logo等比例的宽度
  function getImgWidth(h) {
    var oWidth = 448;
    var oHeight = 133;
    var w = h * oWidth / oHeight;
    return w.toFixed(0);
  }
});
